// src/pages/learning/doc/DockerBaseView.tsx
...
// --- CORE LOGIC HOOK ---
// FIX: Import the dedicated Docker hook
import { useDockerModuleView } from "@/components/hooks/useDockerModuleView.tsx";
// FIX: Import the Docker data
import { DOCKER_COURSE_MODULES, QUIZ_QUESTIONS } from "@/components/data/DockerCourseData.tsx";
// FIX: Import the reusable Quiz component
...
/* -------------------- API CONFIG AND HELPERS (Copied from FullStackBaseView) -------------------- */
...
const updateCourseProgress = async (
    ...
    try {
        ...
        if (response.ok) {
            ...
            // 🛑 NOTE: This ensures the Learning dashboard component refetches the latest state
            return data;
        } else {
...
// --- INTERFACES & MOCK UI COMPONENTS (Reused) ---
...
// MOCK DATA: Needs to match the static plan title defined in Learning.tsx
const MOCK_RAW_PROGRESS: CourseProgress[] = [
    { id: 4, course_name: "Docker and Kubernetes Deep Dive", total_modules: 2, completed_modules: [], progress_percent: 0, started: false, is_completed: false, last_updated: "" },
];
...
// ... (Button, Progress, and pulseVariant components omitted for brevity)
...
// --- MAIN COMPONENT ---
export const DockerBaseView: React.FC<BaseCourseViewProps> = ({
    onBack,
    planTitle,
    initialModuleId,
}) => {

    // 🛑 The expected API course name (Must match Django's DEFAULT_COURSES)
    const API_COURSE_NAME = "Docker and Kubernetes Deep Dive";
    ...
    // NOTE: Use the correct API_COURSE_NAME for local lookups.
    const courseData = rawBackendProgress.find(c => c.course_name === API_COURSE_NAME);
    const currentCourseId = courseData?.id || 4; // Default to ID 4 for Docker

    // --- Call the Docker Hook ---
    const {
        currentModule,
        currentSubmodule,
        currentSubmodules,
        ...
        handleBackToContent,
        handleQuizComplete,
    } = useDockerModuleView(initialModuleId, onBack, rawBackendProgress, currentCourseId);


    // 1. --- WRAPPER: Handle Quiz Complete and Call 'add_module' API ---
    const handleModuleQuizCompleteAPI = useCallback((passed: boolean) => {

        // Find the numerical module ID (e.g., m-d101 -> 101)
        ...
        if (passed && numericalModuleId) {
            // Force the API call to use the known, correct string (API_COURSE_NAME).
    ...
    }, [handleQuizComplete, currentModuleId]);

    // FIX: Redefined sidebarContent as an IIFE
    const sidebarContent = (() => (
        <div className="w-full bg-white dark:bg-gray-900 p-4 space-y-2 h-full overflow-y-auto">
            ...
            <ul className="space-y-1">
                {DOCKER_COURSE_MODULES.map((mod) => { // FIX: Use DOCKER_COURSE_MODULES
                    const index = DOCKER_COURSE_MODULES.findIndex((m) => m.id === mod.id);
                    const prereqId = index > 0 ? DOCKER_COURSE_MODULES[index - 1].id : null;
                            ...
                            {isModuleFullyComplete ? (
                                currentModule.nextModuleId ? (
                                    ...
                                    >
                                        ...
                                        >
                                            <Play className="mr-2 h-4 w-4" /> Go to Next Module: {DOCKER_COURSE_MODULES.find(m => m.id === currentModule.nextModuleId)?.title}
                                        </Button>
                                    </motion.div>
                                ) : (
