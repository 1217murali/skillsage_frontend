// src/pages/system_design/SystemDesignBaseView.tsx
// This component is modeled directly after DSABaseView.tsx to handle the System Design course.
...
// --- CORE LOGIC HOOK ---
// Assume a parallel hook exists for System Design, or use the DSA one if it's generic
// NOTE: You must create this hook: '@/hooks/useSystemDesignModuleView.tsx'
import { useCourseModuleView } from "@/components/hooks/useCourseModuleView.tsx"; // TEMPORARY: Use DSA hook for structure, replace with System Design hook later
import { ALL_COURSE_MODULES, QUIZ_QUESTIONS } from "@/components/data/SystemDesignCourseData.tsx"; // NOTE: You must create this data file
// --- Import the dedicated System Design Quiz component ---
import { ModuleQuiz } from "@/components/ModuleQuiz.tsx"; // NOTE: You must create this component
 ...
 * Duplicated logic from DSABaseView for immediate functionality.
 */
const updateCourseProgress = async (
    ...
    if (action === 'add_module') {
        ...
        try {
            ...
            if (courseToUpdate) {
                ...
                // CHECK: If the module is already in local storage, skip everything and return.
                ...
                // This update is crucial for the new hook to immediately reflect progress
    ...
    // API Call (only reached if 'start' OR if 'add_module' and not returned early)
    try {
        ...
        if (response.ok) {
            ...
            // In a real app, you might want to fetch the updated progress
            return data;
        } else {
...
// --- INTERFACES & MOCK UI COMPONENTS (Copied for isolation) ---

interface CourseProgress {
    id: number; // The course ID (e.g., 4 for "System Design Interview Prep")
    course_name: string;
...
// MOCK DATA: Adjusted to reference a System Design course (ID 4)
const MOCK_RAW_PROGRESS: CourseProgress[] = [
    { id: 1, course_name: "Full Stack Developer Path", total_modules: 12, completed_modules: [], progress_percent: 0, started: false, is_completed: false, last_updated: "" },
    { id: 4, course_name: "System Design Interview Prep", total_modules: 10, completed_modules: [201, 202], progress_percent: 20, started: true, is_completed: false, last_updated: "" },
];
...
// Re-defining Button, Progress, and pulseVariant locally for a complete, isolated example
// In a real project, these would be imported from a central UI library.
...
export const BaseCourseView: React.FC<BaseCourseViewProps> = ({
    ...
    const courseData = rawBackendProgress.find(c => c.course_name === planTitle);
    // CRITICAL: Set the default course ID to the System Design one (e.g., 4)
    const currentCourseId = courseData?.id || 4;

    // NOTE: For a real System Design course, replace `useDSAModuleView` with `useSystemDesignModuleView`
    const {
        currentModule,
        currentSubmodule,
        currentSubmodules,
        ...
        handleBackToContent,
        handleQuizComplete,
    } = useCourseModuleView(initialModuleId, onBack, rawBackendProgress, currentCourseId);


    // 1. --- WRAPPER: Handle Quiz Complete and Call 'add_module' API ---
    const handleModuleQuizCompleteAPI = useCallback((passed: boolean) => {

        // Find the numerical module ID (e.g., m-sd-201 -> 201)
        const numericalModuleIdMatch = currentModuleId.match(/\d+/);
        const numericalModuleId = numericalModuleIdMatch ? parseInt(numericalModuleIdMatch[0]) : undefined;

        if (passed && numericalModuleId) {
            updateCourseProgress('add_module', planTitle, numericalModuleId);
    ...
    }, [handleQuizComplete, planTitle, currentModuleId]);

    // Sidebar Content Logic - Uses ALL_COURSE_MODULES (System Design Data)
    const sidebarContent = (() => (
        <div className="w-full bg-white dark:bg-gray-900 p-4 space-y-2 h-full overflow-y-auto">
            ...
            <h3 className="text-lg font-bold mb-3 mt-4">System Design Modules</h3>
            <ul className="space-y-1">
                {ALL_COURSE_MODULES.map((mod) => {
                    ...
                    const isCompleted = getModuleProgress(mod.id)?.isQuizCompleted;
                ...
                <li className="flex items-center p-3 rounded-lg text-sm text-gray-400 dark:text-gray-600">
                    ...
                    <span>Final Project/Case Study</span>
                </li>
            </ul>
        </div>
    ))();
        ...
        >
            ...
            >
                <div className="max-w-4xl mx-auto">
                    ...
                    {isQuizActive ? (
                        // --- QUIZ VIEW (CRITICALLY using the imported ModuleQuizSystemDesign component) ---
                        <motion.div
                        ...
                        >
                            ...
                            <ModuleQuiz // <-- USE THE IMPORTED SD QUIZ COMPONENT
                        ...
                        <React.Fragment>
                            ...
                            <div className="prose dark:prose-invert max-w-none text-gray-700 dark:text-gray-300">
                                {currentSubmodule?.content || <p className="text-lg mt-8 text-gray-500">This System Design module content is currently being drafted. Please check back soon!</p>}
                            </div>
